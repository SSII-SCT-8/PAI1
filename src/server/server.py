...
from ..common.crypto import truncate_for_log, verify_hmac
...
class IntegrityServer:
                ...
                try:
                    ...
                    with self.connections_lock:
                        if self.active_connections >= MAX_CONNECTIONS:
                            logger.warning(
                                f"Conexión rechazada (máximo alcanzado): {client_address}"
                            )
                            client_socket.close()
                            continue
                    ...
                    logger.info(
                        f"Nueva conexión desde {client_address} (activas: {self.active_connections})"
                    )
    ...
    def _handle_client(self, client_socket: socket.socket, client_address: tuple):
        ...
        try:
            while self.running:
                try:
                    msg_dict = receive_message(
                        client_socket, timeout=CLIENT_MESSAGE_TIMEOUT
                    )
                ...
                try:
                    response = self._process_message(
                        msg_dict, client_ip, session_username
                    )
            ...
            with self.connections_lock:
                self.active_connections -= 1
            logger.info(
                f"Conexión cerrada: {client_ip} (activas: {self.active_connections})"
            )

    def _process_message(
        self, msg_dict: dict, client_ip: str, session_username: Optional[str]
    ) -> dict:
        ...
        try:
            ...
            if msg_type == "LOGIN":
                ...
                if not session_username or session_username != username:
                    ...
                    return create_error_response(
                        "AUTH_ERROR", "No hay sesión activa para cerrar"
                    )
                ...
                return create_error_response(
                    "UNKNOWN_TYPE", f"Tipo de mensaje desconocido: {msg_type}"
                )
    ...
    def _handle_register(
        ...
        try:
            ...
            try:
                ...
                return create_error_response(
                    "REPLAY_ATTACK", "Solicitud de registro duplicada"
                )
