...
from .config import LOG_DIR, LOG_LEVEL, LOG_TO_CONSOLE, LOG_TO_FILE
...
class InteractiveClient:
    ...
    def _handle_transaction(self):
        ...
        if response.get("success"):
            ...
            if data:
                ...
                print(
                    f"  {data.get('from_account')} → {data.get('to_account')}: {data.get('amount')}"
                )
        else:
            print(f"❌ Error: {response.get('message')}")
    ...
    def _handle_replay_attack(self):
        ...
        if resp2.get("success"):
            print("⚠️  VULNERABLE: El servidor aceptó el mensaje repetido!")
            print(f"   {resp2.get('message')}")
        else:
            print("✓ PROTEGIDO: El servidor rechazó el replay")
    ...
    def _handle_mitm_attack(self):
        ...
        response = self.api.send_mitm_attack(
            from_account, to_account, amount, tampered_amount
        )

        print("\n--- RESULTADO ---")
        if response.get("success"):
            print("⚠️  VULNERABLE: El servidor aceptó el mensaje modificado!")
            print(f"   {response.get('message')}")
        else:
            print("✓ PROTEGIDO: El servidor rechazó el mensaje modificado")
