...
def canonicalize_message(msg_dict: Dict[str, Any]) -> bytes:
    """Serializa mensaje sin 'mac' con sort_keys para HMAC determinista."""
    canonical_dict = {k: v for k, v in msg_dict.items() if k != "mac"}
    canonical_json = json.dumps(
        canonical_dict, sort_keys=True, separators=(",", ":"), ensure_ascii=False
    )
    return canonical_json.encode("utf-8")


def send_message(sock: socket.socket, data: Dict[str, Any]) -> None:
    """Envía un mensaje con framing [4 bytes length][JSON payload]."""
    try:
...
def create_error_response(code: str, message: str) -> Dict[str, Any]:
    ...
    return {"type": "ERROR", "code": code, "message": message, "success": False}
...
def create_success_response(
    ...
    response = {"type": "RESPONSE", "message": message, "success": True}
    if data:
        response["data"] = data
    return response
